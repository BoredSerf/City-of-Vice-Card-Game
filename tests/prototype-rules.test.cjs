...
test('opening partner choices clearly show every crew bonus', () => {
  ...
  assert.ok(html.includes("escapeHTML(CREW_BONUSES[c.crew])"));
  assert.match(html, /\.v-opening-caption \.v-opening-bonus\{/);
  assert.ok(html.includes("'The Docks':'When you pay to draw, you may draw 2 cards, then discard 1.'"));
});
